...
module enc_dec #(
    ...
    input [127:0] nonce,
    input [127:0] tag_i,
    input [input_len-1:0] block_i,
    input [a_len-1:0] a_data,
    output [input_len-1:0] block_o,
    output [127:0] tag_o,
  ...
  logic [input_len:0] block_i_reord;

  logic [63:0] tag_0, tag_1;

  assign tag_o = {tag_0, tag_1};
  ...
  order_and_pad #(
      .LEN(input_len)
  ) ord_pad_impl_block_i (
      .i_data(block_i),
      .o_data(block_i_reord)
  );
  ...
  localparam N_BLOCKS = (input_len / (rate << 3)) + 1;
  genvar i;


  logic [(N_BLOCKS*(rate<<3))-1:0] block_o_ord;
  logic [0:0] reg_block_o_cl[N_BLOCKS-1:0];
  logic [0:0] reg_block_o_w[N_BLOCKS-1:0];
  logic [(rate<<3)-1:0] reg_block_o_din[N_BLOCKS-1:0];
  logic [(rate<<3)-1:0] block_o_non_ord[N_BLOCKS-1:0];
  ...
  generate
    for (i = 0; i < N_BLOCKS; i++) begin
      register #(
          ...
          .cl(reg_block_o_cl[i]),
          .w(reg_block_o_w[i]),
          .din(reg_block_o_din[i]),  //& ((2 << plaintext_len) - 1)),
          .dout(block_o_non_ord[i])
      );

      reorder #(
          .LEN(rate << 3)
      ) ord_pad_impl_block_o (
          .i_data(block_o_non_ord[N_BLOCKS-i-1]),
          .o_data(block_o_ord[(i*(rate<<3))+:(rate<<3)])
      );
    end
  endgenerate
  ...
  trail_0s_bytes #(
      ...
      .i_data(block_o_ord >> ((N_BLOCKS * (rate << 3)) - input_len)),
      .o_data(block_o)
  );
  ...
  always_comb begin
    ...
    case (current_state)
      ...
      PLAINTEXT_BLOCK: begin
        custom_state_ascon_din[0] = state_ascon_dout[0] ^ block_i_reord[(generic_counter_o<<6)+:64];
        reg_block_o_din[generic_counter_o][63:0] = state_ascon_dout[0] ^ block_i_reord[(generic_counter_o<<6)+:64];

        if (rate == 16) begin
          aux_var = block_i_reord >> ((generic_counter_o) << 7);

          custom_state_ascon_din[0] = state_ascon_dout[0] ^ aux_var[63:0];
          reg_block_o_din[generic_counter_o][63:0] = state_ascon_dout[0] ^ aux_var[63:0];
          custom_state_ascon_din[1] = state_ascon_dout[1] ^ aux_var[127:64];
          reg_block_o_din[generic_counter_o][127:64] = state_ascon_dout[1] ^ aux_var[127:64];
        ...
        custom_state_ascon_w[0] = 1;
        reg_block_o_w[generic_counter_o] = 1;
        ...
        if (((generic_counter_o + 1) * (rate << 3)) < (input_len + 1)) begin
          next_state = ASCON_PERMUTATION_B_0;
          r_jmp_state_w = 1;
