/**
 * File              : processing_ciphertext_phase.sv
...
module processing_ciphertext_phase #(
    parameter a = 12,
    parameter b = 8,
    parameter rate = 16
) (
  ...
  logic r_block_o_cl;
  logic r_block_o_w;
  logic [(rate<<3)-1:0] r_block_o_din;
  register #(
      .DATA_WIDTH(rate << 3)
  ) r_block_o (
      .clk(clk),
  ...
  always_comb begin
    ...
    case (current_state)
      ...
      XOR_DATA: begin
        if (rate == 8) begin
          state_din[0] = block_i[63:0];
          r_block_o_din = state_dout[0] ^ block_i[63:0];
          state_w[0] = 1;
        end else if (rate == 16) begin
          state_din[0] = block_i[63:0];
          state_din[1] = block_i[127:64];
          r_block_o_din = {(state_dout[1] ^ block_i[127:64]), (state_dout[0] ^ block_i[63:0])};
          state_w[0] = 1;
          state_w[1] = 1;
        end else begin
          next_state = ERROR;
