...
def ascon_hash(message, variant="Ascon-Hash256", hashlength=32, customization=b""):
    ...
    if variant == "Ascon-CXOF128":
        assert len(customization) <= 256
    else:
        assert len(customization) == 0
    a = b = 12  # rounds
    rate = 8  # bytes
    ...
    # Initialization
    iv = (
        to_bytes([versions[variant], 0, (b << 4) + a])
    ...
    # Customization
    if customize:
        z_padding = to_bytes([0x01]) + zero_bytes(
            rate - (len(customization) % rate) - 1
        )
...
def ascon_mac(key, message, variant="Ascon-Mac", taglength=16):
    ...
    if variant == "Ascon-PrfShort":
        ...
        IV = to_bytes(
            [len(key) * 8, len(message) * 8, a + 64, taglength * 8]
        ) + zero_bytes(4)
        ...
        # Finalization (Squeezing)
        T = int_to_bytes(S[3] ^ bytes_to_int(key[0:8]), 8) + int_to_bytes(
            S[4] ^ bytes_to_int(key[8:16]), 8
        )
        return T[:taglength]
        ...
        S = bytes_to_state(
            to_bytes([len(key) * 8, rate * 8, a + 128, a - b])
            + tagspec
            + key
            + zero_bytes(16)
        )
        ...
        # Message Processing (Absorbing)
        m_padding = to_bytes([0x01]) + zero_bytes(
            msgblocksize - (len(message) % msgblocksize) - 1
        )
        m_padded = message + m_padding
...
def ascon_process_associated_data(S, b, rate, associateddata):
    ...
    if len(associateddata) > 0:
        a_padding = to_bytes([0x01]) + zero_bytes(
            rate - (len(associateddata) % rate) - 1
        )
        print("a_padding = {0}".format(bytes_to_hex(a_padding)))

        a_padded = associateddata + a_padding

        print("associateddata = {0}".format(bytes_to_hex(associateddata)))
        print("a_padded = {0}".format(bytes_to_hex(a_padded)))
        print("rate = {0}".format(rate))

        for block in range(0, len(a_padded), rate):

            print("block = {0}".format(
                bytes_to_hex(a_padded[block: block + 8])))
...
def ascon_process_plaintext(S, b, rate, plaintext):
    ...
    print("plaintext = {0}".format(bytes_to_hex(plaintext)))
    print("p_padded = {0}".format(bytes_to_hex(p_padded)))
    print("p_padding = {0}".format(bytes_to_hex(p_padding)))
...
def ascon_process_ciphertext(S, b, rate, ciphertext):
    ...
    c_padx = zero_bytes(c_lastlen) + \
        to_bytes([0x01]) + zero_bytes(rate - c_lastlen - 1)
    ...
    S[0] = (S[0] & bytes_to_int(c_mask[0:8])
            ) ^ Ci[0] ^ bytes_to_int(c_padx[0:8])
    S[1] = (S[1] & bytes_to_int(c_mask[8:16])
            ) ^ Ci[1] ^ bytes_to_int(c_padx[8:16])
    if debug:
...
def demo_aead(variant="Ascon-AEAD128", bytes_used=8):
    ...
    associateddata = bytes.fromhex(
        "23a7711a8133287637ebdcd9e87a1613e443df789558867f5ba91faf7a024204")
    # get_random_bytes(len_bytes)  # b"ascon"
    ...
    print(
        "python3 ascon_aead.py 0x{0} 0x{1} 0x{2} 0x{3} {4}".format(
            bytes_to_hex(plaintext),
            bytes_to_hex(key),
            bytes_to_hex(nonce),
            bytes_to_hex(associateddata),
            len_bytes * 8,
        )
    )
...
def demo_hash(variant="Ascon-Hash256", hashlength=32):
    ...
    demo_print(
        [("message", message), ("customization", customization), ("tag", tag)])
...
if __name__ == "__main__":
    bytes_used = int(sys.argv[1])
    demo_aead("Ascon-AEAD128", bytes_used)
