#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# File              : ascon_processing_ciphertext_phase_test.py
...
async def test_ciphertext_data(dut, expected_State, C_array, num_blocks):
    dut.rst.value = 0
    dut.start.value = 1
    S_dut = [0, 0, 0, 0, 0]

    P_array = [0] * (num_blocks)

    for i in range(0, 5):
        S_dut[i] = int(dut.state_dout[i].value)

    dut.block_i.value = C_array[0]
    await n_cycles_clock(dut, 1)
    for n in range(0, num_blocks):
        dut._log.info(
            "cycle {0} with numblocks = {2} in test with data={1}".format(
                n, hex(C_array[n]), num_blocks
            )
        )
        ...
        P_array[n] = int(dut.block_o.value)
    ...
    for i in range(0, num_blocks):
        dut._log.info("P_array[{0}] = {1}".format(i, hex(P_array[i])))
...
async def test(dut, index=0):
    ...
    C_value = 0
    for i in range(0, n):
        C_data = (random.getrandbits(int(dut.rate.value) * 8)) | (
            (1 << (int(dut.rate.value) * 8) - 1)
        )
        C_array[i] = C_data
        C_value = C_value + (C_data << (int(dut.rate.value)) * (i * 8))
    ...
    expected_plaintext = ascon_process_ciphertext(
        S,
        int(dut.b.value),
        int(dut.rate.value),
        C_value.to_bytes((n * int(dut.rate.value)), "little"),
    )

    setup_dut(dut, S_init)
    await rst_function_test(dut)
    await test_ciphertext_data(dut, S, C_array, n)

    dut._log.info(bytes_to_hex(expected_plaintext))
